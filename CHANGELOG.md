# 2.1.0 - Audio Modes, Distance by volume, Increased "Loudness"! [V80/81]
...
* Added dsp Filters for increased volume 100 <, so that 100-200 is more noticeable
...
* Added new dependency ```Lordfirespeed-YoutubeDLSharp```

#### **Fixes:**

* Fixed `Audio source failed to initialize audio spatializer.` Log spam, vanilla seems to still present this every so often
...
* Changed Auto download of ffmpeg and yt-dlp to use ```Lordfirespeed-YoutubeDLSharp```, danke 753 for bringing this package to my attention, this reduces the inflated FFMPEG download size to around 137MB.
...
* Changed README to reflect compatibility with V80/V81! ~~(So far testing has been conducted on the Steam Public beta branch, this means that compatibility status is subject to change when the full release is released!)~~ 
...
* If you encounter any issues [submit an issue report](https://github.com/Henreh1/BoomBoxOverhaul/issues)
